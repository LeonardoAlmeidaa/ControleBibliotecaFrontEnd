      ...
      <div class="row align-items-center" v-if="filterToggle">
        <div class="col-xs-9 col-sm-9 col-md-9 col-xxl-9">
          ...
          <TheDateRange
            ...
            :label="object.label"
          />
          <TheDate v-else v-model="inputValue" :ref="object.ref" :label="object.label" />
        </div>
        <div class="col-xs-3 col-sm-3 col-md-3 col-xxl-3 mt-2 text-end">
          <TheButton type="button" color="primary" label="Buscar" icon="bi bi-search" @click="emitConfirm" />
          <TheButton type="button" color="outline-danger" label="Limpar" icon="bi bi-x-lg" @click="emitClear" />
        </div>
      </div>
    </TheTransition>
    ...
    async filterAll() {
      if (await checkSession()) {
        ...
        if (this.inputValue != '' || this.inputValue != null) {
          // this.items = await search(`${this.object.route}/search/${subRoute}`, params)
          const filterObject = {
